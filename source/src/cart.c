...
bool cart_mbc5() {
    // types from 0x19 à 0x1E
    return BETWEEN(ctx.header->type, 0x19, 0x1E);
}
...
bool cart_load(char* cart) {
    ...
    // 0x0143 flag GBC
    u8 cgb_flag = ctx.rom_data[0x0143];
    const char* cgb_support;

    if (cgb_flag == 0x80) {
        cgb_support = "Compatible GB/GBC";
    }
    else if (cgb_flag == 0xC0) {
        cgb_support = "GBC Only";
    }
    else {
        cgb_support = "Game Boy Classic";
    }

    char title_buf[16] = { 0 };
    strncpy_s(title_buf, sizeof(title_buf), (char*)&ctx.rom_data[0x0134], 15);// copy to avoid crash if reread
    ...
    printf("\t Title    : %s\n", title_buf); 
    printf("\t CGB Supp : %2.2X (%s)\n", cgb_flag, cgb_support);
...
u8 cart_read(u16 address) {
    ...
    if (address < 0x4000) {
        return ctx.rom_data[address];// Bank 0
    }
...
void cart_write(u16 address, u8 value) {
    if (cart_mbc5()) {
        if (address < 0x2000) {
            ...
            ctx.rom_bank_value = (ctx.rom_bank_value & 0x0FF) | ((value & 0x01) << 8);
            ctx.rom_bank_x = ctx.rom_data + (0x4000 * ctx.rom_bank_value);
        }
        else if (address < 0x6000) {
            ctx.ram_bank_value = value & 0x07;
            ctx.ram_bank = ctx.ram_banks[ctx.ram_bank_value];
            ctx.rumble_active = (value & 0x08) != 0;
        }
